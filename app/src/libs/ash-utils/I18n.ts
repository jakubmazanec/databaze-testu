 ...
 * @returns {string}
 */
let buildKey = (literals: Array<string>) => {
	let stripType = (s: string) => s.replace(TYPE_REGEX, '');
	let lastPartialKey = stripType(literals[literals.length - 1]);
	let prependPartialKey = (memo: string, curr: string, i: number) => `${stripType(curr)}{${i}}${memo}`;
 ...
 * @returns {[type]}
 */
let buildMessage = (string: string, ...values: Array<any>) => string.replace(/{(\d)}/g, (_, index) => values[Number(index)]);
 ...
 * @returns {string}
 */
let localizeString = (locale: string, string: any) => string.toLocaleString(locale);
 ...
 * @returns {string}
 */
let localizeCurrency = (locale: string, string: any, currency: string) => string.toLocaleString(locale, {
	style: 'currency',
	currency
});
 ...
 * @returns {string}
 */
let localizeNumber = (locale: string, string: any, fractionalDigits: number) => string.toLocaleString(locale, {
 ...
 * @returns {Object}
 */
let extractTypeInfo = (literal: string) => {
 ...
 * @returns {[type]}
 */
let localize = (value: any, i18n: any, {type, options}: {type: string; options: any}) => {
	let localizedValue;
...
export default class I18n {
	 ...
	 * @returns {string}
	 */
	translate(literals: Array<string>, ...values: Array<any>) {
		let translationKey = buildKey(literals);
		let translationString = this.getTranslationString(translationKey);
	...
	getTranslationString(translationKey: string) {
		if (this.strings[translationKey] && this.strings[translationKey][this.locale]) {
			return this.strings[translationKey][this.locale];
		}
