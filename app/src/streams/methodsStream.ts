...
export enum MethodsStreamStatus {
	idle = 'IDLE',
	loading = 'LOADING'
}
...
let methodsStream: Stream<MethodsStreamValue> = new Stream({
	methods: [],
...
methodsStream.push((async () => {
	...
	return {
		methods: response.data.methods.map((method: Method) => ({...method, status: MethodStatus.idle})),
		status: MethodsStreamStatus.idle
	};
})());

let cloneMethodsStreamValue = (methodsStreamValue?: {}): MethodsStreamValue => Object.assign({}, methodsStream.value, methodsStreamValue);
let changeMethodStatus = (methods: Array<Method>, methodUuid: string, newStatus: MethodStatus) => {
...
methodsStream.combine(async (self, changed) => {
	...
	if (changed.includes(updateMethodActionStream)) {
		...
		if (!response || response.errors && response.errors.length) {
			...
			let updatedMethod = {...response.data.updateMethod.method, status: MethodStatus.idle};
...
(global as any).methodsStream = methodsStream;
