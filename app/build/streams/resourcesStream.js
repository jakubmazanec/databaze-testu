...
export var ResourcesStreamStatus;
(function (ResourcesStreamStatus) {
    ResourcesStreamStatus["idle"] = "IDLE";
    ResourcesStreamStatus["loading"] = "LOADING";
})(ResourcesStreamStatus || (ResourcesStreamStatus = {}));
let resourcesStream = new Stream({
    resources: [],
...
resourcesStream.push((async () => {
    ...
    return {
        resources: response.data.resources.map(resource => Object.assign({}, resource, { status: ResourceStatus.idle })),
        status: ResourcesStreamStatus.idle
    };
})());
let cloneResourcesStreamValue = resourcesStreamValue => Object.assign({}, resourcesStream.value, resourcesStreamValue);
let changeResourceStatus = (resources, resourceUuid, newStatus) => {
...
resourcesStream.combine(async (self, changed) => {
    ...
    if (changed.includes(updateResourceActionStream)) {
        ...
        if (!response || response.errors && response.errors.length) {
            ...
            let updatedResource = Object.assign({}, response.data.updateResource.resource, { status: ResourceStatus.idle });
...
global.resourcesStream = resourcesStream;
